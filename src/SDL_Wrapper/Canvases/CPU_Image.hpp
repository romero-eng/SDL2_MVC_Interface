...
namespace SDL{ namespace CPU_Images { using CPU_Image = SDL_Surface; } }

namespace SDL
{
    namespace CPU_Images
    {
        CPU_Image* LoadFromFile(fs::path&& image_path);

        CPU_Image* LoadFromFile(fs::path& image_path);

        void FreeSurface(CPU_Image* surface);
    }
}
